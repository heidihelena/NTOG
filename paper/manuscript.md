...
**Results.** Port validation passed exactly (Δ = 0 over 5,000 random inputs; every Brock/Mayo coefficient `exp(β)` matched its published odds ratio). On the synthetic cohort the Nordic composite was statistically indistinguishable from Brock and Mayo on static discrimination (AUROC ≈ 0.70–0.72; pairwise Δ ≈ 0, *p* > 0.7) but reranked fast-growing nodules upward as designed (growth-tracking *r* = 0.24). On a 6,000-nodule three-axis cohort, single-axis AUROCs were CRS 0.72, protein 0.74, deep-learning CT 0.83; **all three combined reached 0.86** (EpiNet honest model 0.85, well-calibrated, slope 1.15). The protein axis added discrimination **even on top of the CT axis** (+0.03) and uniquely caught 183 malignancies that *both* imaging axes missed at fixed 85% specificity (union sensitivity 78% vs 42% for CRS alone). Applying the protein axis to **only the contested 40% grey zone** recovered roughly **half** the full-cohort gain. EpiNet contestability ranked value-of-information as CT > protein > clinical.

**Conclusion.** Squeezing the existing imaging-clinical features cannot exceed an information ceiling (~0.72 in the hard, indeterminate regime). Breaking it requires an **orthogonal axis** — a blood biomarker or a deep-learning imaging signature — and the resource-efficient move is to deploy it on the **grey zone**, not the whole cohort. Treating model–threshold coupling, decision geometry, and time as first-class objects gives guideline groups a concrete, quantitative language for pathway design.
...
- **Deep-learning CT (Sybil-like)** reads the whole image — semantic *plus* sub-visual texture (0.86 target; 0.83 achieved here).
...
On the synthetic cohort (n = 117, 33% malignant): AUROC Brock 0.703 (95% CI 0.602–0.796), Mayo 0.716 (0.621–0.809), **Nordic/NTOG 0.711 (0.611–0.805)**. All pairwise differences were null (Δ ≈ 0, *p* > 0.7). The Nordic score's *distinct* behaviour was **growth reranking**: its rank shift versus Brock tracked growth rate (*r* = 0.24) — faster-growing nodules ranked higher, exactly as the growth domain intends. *Interpretation: the Nordic score's contribution is the temporal/growth axis, not a static-discrimination gain — a more defensible and more interesting claim than "ours is better."*

### 4.3 Breaking the ceiling needs an orthogonal axis (Figure 1)
On the 6,000-nodule three-axis cohort (5-fold CV AUROC):

| Model | AUROC |
|---|---|
| Clinical only (CRS-like) | 0.720 |
| Protein only (4MP-like) | 0.740 |
| Deep-learning CT only (Sybil-like) | 0.826 |
| CRS + protein (all) | 0.778 |
| CRS + DL-CT (all) | 0.826 |
| **All three axes** | **0.857** |

Protein added **+0.031 even on top of the CT axis** (0.826 → 0.857): two imaging axes do not substitute for the blood axis. *Interpretation: orthogonal information is additive; more samples on the same features would only have tightened 0.72, not raised it.*
...
At fixed 85% specificity, among true malignancies: the protein axis uniquely caught **183** cancers that *both* CT axes missed; the DL-CT axis uniquely caught 266; CRS-only 35 (small — CT already reads the image); 353 were missed by all. **Union sensitivity 78% vs 42% for CRS alone.** *Interpretation: the value of a new axis is the cancers it catches that the others cannot — visible directly as the non-overlapping slices.*

### 4.6 Honest evaluation confirms it (Figures 3–4)
EpiNet's honest RandomForest over the three axes reached AUROC **0.85** (iteration mean 0.849 ± 0.008) and was **well-calibrated** (Brier 0.13, calibration slope **1.15**, intercept ≈ 0). Contestability ranked **value-of-information** as deep-learning CT (0.41) > protein (0.31) > clinical (0.28): the CT axis resolves the most contested calls, but the protein axis is a close second *and* the cheapest orthogonal one. *Interpretation: the combined model is genuinely informative and trustworthy on this cohort — not an over-fitted in-sample number.*
...
Our simulation reproduces the empirically reported pattern — each axis adds, the protein axis adds *even on top of* the imaging DL axis, and the cheapest deployment targets the grey zone — and turns it into a tunable figure for guideline discussion (sandbox tab 4).

**A concrete pathway proposal.** Stage 1: the imaging-clinical model confidently resolves the easy majority. Stage 2: an orthogonal axis (protein panel, deep-learning CT re-read, or a deliberately-timed short-interval re-scan to read growth) is applied **only to the contested grey zone**, where value-of-information is highest. This optimises *net benefit*, not AUROC, and is affordable for a real Nordic pathway. EpiNet's contestability identifies exactly which nodules qualify.
...
**Figure 3 — EpiNet calibration.** The honest three-axis model is well-calibrated (slope ≈ 1.15, Brier 0.13): predicted risks track observed frequencies, so the AUROC reflects real signal rather than leakage.
...
10. EpiNet — the Epistemic Network toolkit. https://github.com/heidihelena/epinet.
