...
**A reproducible decision-analytic simulation framework for pathway design**
*(supported by a browser sandbox — Nordic Nodule Pathway Sandbox v3 — and an honest-evaluation toolkit, EpiNet)*

*Heidi H. Andersén¹²  ·  Nordic Thoracic Oncology Group (NTOG)*
¹ Tampere University · ² University of Turku · Tools powered by Vahtian

> **Positioning.** *This is **not** a validation of clinical performance; it is a transparent, reproducible **simulation** showing how model choice, thresholds, temporal ambiguity, and orthogonal information axes reshape pathway decisions.* The contribution is a way to reason honestly about **where an added test has value** — not a claim that any specific biomarker or model "works."
...
**Conclusion.** Squeezing the existing imaging-clinical features cannot exceed an information ceiling set by feature–outcome mutual information — **in our hard synthetic regime that level is ~0.72** (real screen-detected cohorts sit higher; Brock externally ~0.85–0.90 on easier nodules). Breaking it requires an **orthogonal axis** — a blood biomarker or a deep-learning imaging signature — and a **hypothesis worth prospective testing** is to deploy it on the **grey zone**, not the whole cohort. Treating model–threshold coupling, decision geometry, and time as first-class objects gives guideline groups a concrete, quantitative language for pathway design.
...
This manuscript describes two open tools built around these ideas and uses them to ask a concrete question raised in NTOG working-group discussion: **in our hard, indeterminate synthetic regime the imaging-clinical model plateaus around AUROC 0.72 — what would it take to do better?** (We use 0.72 because it is the level we calibrate the synthetic clinical axis to; real screen-detected nodule cohorts sit higher.) The answer, *within this framework*, is not more data on the same features (that buys precision, not a higher ceiling) but an **orthogonal information axis** — and the cheapest way to capture its benefit is to spend it on the grey zone.
...
Two consequences follow directly from the construction, and should be read as such. (i) `CRS + DL-CT = DL-CT alone` (both 0.826) because, by construction, the Sybil axis already contains the clinical signal `Uc` — i.e. **clinical is redundant with Sybil**, not informative on top of it. (ii) The protein axis adds **+0.031 on top of the CT axis** (0.826 → 0.857) precisely because `Up` is, by construction, unseen by either imaging axis. To be precise about the orthogonality structure (the axes are *not* all mutually orthogonal): **the protein component is orthogonal to both imaging components; the Sybil-like axis is partly redundant with the clinical-imaging axis (it shares `Uc`) and partly orthogonal through a sub-visual texture component (`Us`)** — which is clinically realistic for a deep-learning CT model that absorbs both visible semantic features and sub-visual texture. *Interpretation: the simulation makes explicit a hypothesis — that an axis is worth adding exactly to the extent it is orthogonal — and quantifies its size under that assumption; it does not establish the assumption.*

### 4.4 You don't need to test everyone (the grey-zone strategy)
Measuring the protein axis on **only the contested 40%** (the clinical grey zone) recovered **0.029 of the 0.058** full-cohort AUROC gain — about **half the benefit at 40% of the blood draws**. **This is an illustrative approximation, not a prospective two-stage design:** the clinical+protein model is trained in cross-validation with protein available for *everyone*, and its protein-enhanced prediction is then *substituted in* for the contested observations only. It does not yet model the real policy — defining the grey zone within each training fold, fitting the second-stage model on grey-zone training observations alone, and applying it only to grey-zone test observations — which a clinical implementation study would need. *Interpretation: spend the expensive axis where the cheap model is uncertain — quantified here as an upper-ish-bound illustration, to be re-estimated under a true two-stage protocol.*

### 4.5 Axes are complementary (Figure 2)
Sets are defined from each **raw simulated axis score** at the 85%-specificity threshold among non-malignant nodules (not from cross-validated model probabilities). At that threshold, among true malignancies: the protein axis uniquely caught **183** cancers that *both* CT axes missed; the DL-CT axis uniquely caught 266; CRS-only 35 (small — by construction the Sybil axis already reads the image); 353 were missed by all. **Union sensitivity 78% vs 42% for CRS alone.** *These are single-seed point estimates without uncertainty intervals, shown to illustrate the structure of complementarity, not to quantify it. Interpretation: the value of a new axis is the cancers it catches that the others cannot — the non-overlapping slices — which here follows from the assumed orthogonality.*

### 4.6 Independent evaluation reproduced the expected synthetic three-axis behaviour (Figures 3–4)
EpiNet's honest model over the three axes reached AUROC **0.85** (iteration mean 0.849 ± 0.008), independently reproduced by a separate stack (RandomForest 0.846, logistic 0.857; see `SCORECARD.md`). **Calibration was reported as an unresolved discrepancy in earlier drafts; the `rms` cross-check now resolves it — and the lesson is that the slope is *model-dependent*, not method-dependent.** The Brier score agrees everywhere (EpiNet 0.131; sklearn 0.135; `rms` 0.130). The slope, however, depends on which estimator is calibrated. The definitive arbiter — **`rms::validate()` on a logistic combined model (B = 500, optimism-corrected) — gives a calibration slope of 0.996** (optimism-corrected C/AUROC 0.857), i.e. the *logistic* combined model is **near-ideally calibrated** (Fig 6). The sub-1 slopes seen earlier (out-of-fold CV 0.94; optimism-corrected bootstrap 0.83) and EpiNet's apparent/iteration 1.15 are all properties of the **RandomForest**, which is mildly over-confident and would benefit from recalibration. So the honest statement is not "the model is mis-calibrated" but "calibration depends on the estimator: the logistic model is well-calibrated; the RandomForest needs recalibration where probabilities matter." Reporting only EpiNet's 1.15 would have implied the wrong direction *for the RF* and missed that a logistic model is essentially ideal — exactly the selective-number trap EpiNet's own honest-evaluation stance is meant to prevent. A **label-permutation null** confirms the discrimination beats chance (observed AUROC 0.857 vs permuted-null mean 0.500, *p* = 0.002 (1/501; observed exceeds all 500 permutations)) — but this tests signal-versus-chance, **not** whether the three axes are genuinely orthogonal; orthogonality is a design assumption here, not a testable output. Contestability ranked **value-of-information** as deep-learning CT (0.41) > protein (0.31) > clinical (0.28) — an *illustrative* ordering that reflects the standardized model on synthetic data, not a biological hierarchy. *Interpretation: the combined model reproduces across tools and beats chance; its trustworthiness claim rests on cross-validation and the permutation null, while the calibration slope is reported with its honest method-dependence.*
...
**Figure 1 — Discrimination ladder.** Cross-validated AUROC for each information axis and their combinations on the 6,000-nodule synthetic cohort. The **simulated clinical-only level** (0.72, a calibrated input) is lifted by the added axes *under the built-in orthogonality assumption*; all three reach 0.86. The dashed line marks the clinical-only level, not a demonstrated ceiling. *Synthetic, calibrated to literature — not real performance.*
...
**Figure 5 — Decision curve (Python `dcurves`).** Net benefit of the three-axis model versus treat-all / treat-none across thresholds; the model is preferred over a wide range. An independent clinical-utility check EpiNet does not itself compute.

**Figure 6 — Calibration, `rms` (logistic three-axis model).** Bootstrap calibration from `rms::calibrate`/`validate` (B = 500): optimism-corrected slope **0.996**, C/AUROC 0.857, Brier 0.130 — the *logistic* combined model is near-ideally calibrated. (This is a different estimator from EpiNet's RandomForest; see §4.6.)

**Figure 7 — Decision curve, `rms`/`dcurves` (R).** Net benefit 0.218 / 0.177 / 0.142 at thresholds 0.10 / 0.20 / 0.30 — agreeing with the Python `dcurves` result (Fig 5), i.e. the clinical-utility finding reproduces across two languages.
